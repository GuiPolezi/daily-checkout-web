...
export async function POST(req: Request) {
  try {
    ...
    console.log("Tentando conectar com o usuário:", process.env.SMTP_USER);
console.log("No Host:", process.env.SMTP_HOST);
// NUNCA dê log na senha real, mas você pode ver o tamanho dela para checar se foi lida:
console.log("Tamanho da senha:", process.env.SMTP_PASS?.length);
    // 1. Configuração do transportador SMTP
    const transporter = nodemailer.createTransport({
      host: process.env.SMTP_HOST,
      port: Number(process.env.SMTP_PORT),
      secure: process.env.SMTP_PORT === "465", // Dinâmico
      auth: {
        user: process.env.SMTP_USER,
        pass: process.env.SMTP_PASS,
      },
      tls: {
        // Isso ajuda se o certificado do seu servidor SMTP for auto-assinado ou tiver problemas de cadeia
